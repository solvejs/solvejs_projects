...
// }
    ...
    handleHoverCartButton = () => {
        if(document.cookie.split(';').find( (item) => item.includes('bsLoggedIn=true'))) {
            this.setState({
                titleForButton: '',
...
function PurchButton(props) {
    ...
    return <button disabled={props.disabled} style={{cursor: 'pointer'}} onMouseOver={props.hover} onChange={props.hover} onClick={props.order} title={props.title}>{props.text}</button>
